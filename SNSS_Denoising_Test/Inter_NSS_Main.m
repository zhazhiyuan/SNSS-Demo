...
function [Imout] = Inter_NSS_Main(InputImage, Par, nSig, patch, Sim, step)
...
for  i  =  1 : NN
    ...
    for  j  =  1 : MM
           ...
           currow              =              row(i);
            ...
            lambda             =               repmat ( 2*sqrt(2)*Par. c2 *nSig^2./(sqrt(s0)+eps),[1, size(A0,2)]); %Generate the weight Eq.(19)
        
             tau               =               lambda * Par.mu2 *nSig ^2;
        
           Alpha               =               soft (A0, tau);  % L0_norm
