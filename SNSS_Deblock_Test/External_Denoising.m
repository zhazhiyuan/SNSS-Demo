...
function  [Exter_Out]       =       External_Denoising  (Exte,  Opts,  model)
...
for     ite   =    1
    ...
    for   j           =         1:length(seg)-1
        ...
        lambdaM       =         repmat(2*sqrt(2)*Opts.c1*Exte.nSig^2./ (sqrt(S)+eps ),[1 Exte.nlsp]);
        ...
        for i         =          1:size(idx,1)
            ...
            b         =          D'*Y;
            % soft threshold
            alpha     =          sign(b).*max(abs(b)-(lambdaM* Opts.mu1*Exte.nSig^2),0);
